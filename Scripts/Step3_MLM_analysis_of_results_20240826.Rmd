...
ggplot(predictions_df, aes(x = Sample, y = predicted_prob, linetype = Index, color = Index)) +
  geom_line(size = 1.2) +  # Use varying colors for the lines
  geom_point(aes(shape = Index), size = 4) +  # Add different shapes for each index
  ...
  scale_color_manual(values = c("black", "grey20", "grey40", "grey60", "grey80"), 
                     labels = labels.index) +  # Use various shades of grey for the lines
  ...
  scale_shape_manual(values = c(16, 17, 18, 19, 15),  # Different shapes: circles, triangles, squares, etc.
                     labels = labels.index) +  # Same labels for linetypes and shapes
  geom_hline(yintercept = 0.8, linetype = "dashed", color = "grey50") +  # Add dashed line at 80%
  geom_hline(yintercept = 0.5, linetype = "solid", color = "grey50")  # Keep the line at 50%
