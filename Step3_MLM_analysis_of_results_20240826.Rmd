...
for(i in 1:length(results)) {
  ...
  # Extrahiere die Werte und setze NA, falls ein Wert fehlt
  # `ifelse` Dieser Ausdruck überprüft, ob ein Wert NULL ist. Wenn ja, wird NA anstelle von NULL eingesetzt. 
  # Dies verhindert, dass weniger als 5 Werte in die Tabelle eingefügt werden.
...
ggplot(overunderfit_long, aes(x = Index, y = correct * 100, group = Index)) +
  ...
  facet_wrap(~ Sample, nrow = 3, ncol = 17, scales = "free_y") +
  scale_fill_manual(values = c("grey0", "grey25", "grey50", "grey75", "grey100")) +
  ylab("% correctly identified model (5 profiles)") +
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  labs(fill = "Fit index") +
  geom_hline(yintercept = 50, linetype = "solid", color = "grey25") +  # Durchgehende Linie bei y = 50%
  geom_hline(yintercept = 33, linetype = "dashed", color = "grey25") +  # Gestrichelte Linie bei y = 33%
  geom_hline(yintercept = 66, linetype = "dashed", color = "grey25") +  # Gestrichelte Linie bei y = 66%
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 8),  # Achsentexte auf der x-Achse rotieren und Schriftgröße anpassen
    strip.text.x = element_text(size = 8),  # Textgröße der Facet-Labels
    strip.background = element_blank(),  # Hintergrund der Facet-Labels entfernen
    axis.title.x = element_blank(),  # X-Achsentitel entfernen (falls benötigt)
    axis.text.y = element_text(size = 8),  # Textgröße der Y-Achsen-Ticks
    panel.grid.major.y = element_line(color = "grey90"),  # Optional: Y-Achsen Gitternetzlinien hinzufügen
    strip.placement = "outside"  # Fit-Index-Beschriftungen nur in der unteren Zeile anzeigen
  )

ggsave("fitpower_2024-09.png", width = 18.5, height = 8, dpi = 600)
